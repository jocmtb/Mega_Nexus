...
def parse_interface(f):
  ...
  INTERFACE_ID=''
  for line in f.splitlines():
    search_new=re.search('^(Ethernet[1-8]/[0-9]+) is up',line)
    if search_new:
    ...
    if search3:
      ...
      if INTERFACE_ID not in dict_interfaces.keys() and ('channel' in INTERFACE_ID or 'thernet' in INTERFACE_ID):
...
def parse_arp(f):
  ...
  lista_vlan={}
  for line in f.splitlines():
    search_vlan=re.search('^(Vlan[0-9]+)\s+(.+)',line)
    if search_vlan:
      #print 'match#3'
      lista_vlan[search_vlan.group(1)]=search_vlan.group(2)
    search_mac=re.search('dynamic',line)
    search_mac2=re.search('[\w\d]+\.[\w\d]+\.[\w\d]+\s+secure',line)
    if (search_mac or search_mac2) and len(line.split())==8:
      #print 'match#1'
      MAC_add=line.split()[2]
      MAC_intf=line.split()[7]
      MAC_vlan='Vlan'+line.split()[1]
      if MAC_vlan in lista_vlan.keys():
         lista_macs.append({'mac_add':MAC_add,'mac_intf':MAC_intf,'mac_ip':'na','mac_vlan':MAC_vlan,'vlan_name':lista_vlan[MAC_vlan]})
      else:
         lista_macs.append({'mac_add':MAC_add,'mac_intf':MAC_intf,'mac_ip':'na','mac_vlan':MAC_vlan,'vlan_name':'None'})
    search_arp=re.search('\d\d:\d\d:\d\d',line)
    if search_arp and len(line.split())==4:
      #print 'match#3'
      MAC_add=line.split()[2]
      MAC_ip=line.split()[0]
...
def parse_igmp(f):
  ...
  for line in f.splitlines():
    ...
    if search_v3 and len(line.split())==6:
      ...
      dict_igmp.append({'mcast_grp':MCAST_GRP,'reporter_ip':REPORTER_IP
                        ,'version':'v3','mcast_src':MCAST_SRC,'intf':line.split()[2]})
  return dict_igmp
...
def parse_mcast(buffer):
   ...
   for x in output_data:
      if len(x.split())==3 and 'VRF name' in x:
...
def Create_Excel_Table_xr(ip1,date1,data):
        ...
        filename='./polls/session_logs/'+ip1+'_'+date1+'_xr_mcast.xlsx'
